...
export function PageHeader({ title, description }: PageHeaderProps) {
  return (
    ...
    >
      <h1 className="font-extrabold text-[40px] text-white">{title}</h1>
      <p className="font-medium text-[16px] text-white">{description ?? ''}</p>
      <div className="flex w-[calc(100%-272px)] max-[639px]:w-[calc(100%-64px)] absolute bottom-0 border-b border-b-[2px] border-brandBlue-200"></div>
    </div>
  )
}
